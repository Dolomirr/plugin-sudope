function sudope -d "Quickly toggle sudo prefix"
  ...
  set -l _sudope_command "sudo"
  if set -q SUDOPE_COMMAND
    set _sudope_command $SUDOPE_COMMAND
  end
  set -l prefix_len (string length -- "$_sudope_command ")
  ...
  set -l escaped_prefix (string escape --style=regex "$_sudope_command")
  set -l command_parts (string match -ir "^(\s*)($escaped_prefix(\s+|\$))?(.*)" $command_buffer[1])
